        ...
        if len(faces) >= 1:
            for i in range(len(faces)):
                try:
                    ...
                    count+=1
                    data = {"image":data, "cam":cam_id, "timestamp": time.time(), "image_number":count}
                    ws.send(json.dumps(data))
                    print("sent from camera : ", cam_id, ", count : ", count)
                    #time.sleep(0.25)
...
ws = create_connection('ws://192.168.29.237:4000/websocket_service/')
print(ws)
...
t3 = threading.Thread(target=run, args=(2, 1000, False, 50))
